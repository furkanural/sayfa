defmodule Sayfa.Blocks.Header do
  ...
  defp render_nav(nav, search_trigger, lang_switcher, page_url) do
    ...
    #{search_trigger}\
    #{lang_switcher}\
            <button id="menu-toggle" onclick="(function(){var m=document.getElementById('mobile-menu'),o=document.querySelector('.menu-open'),c=document.querySelector('.menu-close'),h=m.classList.contains('hidden');m.classList.toggle('hidden');o.classList.toggle('hidden',h);c.classList.toggle('hidden',!h);this.setAttribute('aria-expanded',String(!h))})()" class="md:hidden p-2 -mr-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100" aria-label="Toggle menu" aria-expanded="false" aria-controls="mobile-menu">\
        ...
        <script>document.addEventListener('keydown',function(e){if(e.key==='Escape'){var m=document.getElementById('mobile-menu');if(m&&!m.classList.contains('hidden')){m.classList.add('hidden');document.querySelector('.menu-open').classList.remove('hidden');document.querySelector('.menu-close').classList.add('hidden');var b=document.getElementById('menu-toggle');b.setAttribute('aria-expanded','false');b.focus()}var l=document.getElementById('lang-menu');if(l&&!l.classList.contains('hidden')){l.classList.add('hidden');var t=document.getElementById('lang-toggle');t.setAttribute('aria-expanded','false');t.focus()}}})</script>\
    """
  end
