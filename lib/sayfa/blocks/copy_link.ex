defmodule Sayfa.Blocks.CopyLink do
  ...
  def render(_assigns) do
    ...
    <div class="mt-10 pt-6 border-t border-slate-200 dark:border-slate-700/50 flex items-center gap-3">\
      <button onclick="(function(btn){navigator.clipboard.writeText(window.location.href).then(function(){btn.querySelector('span').textContent='Copied!';setTimeout(function(){btn.querySelector('span').textContent='Copy link'},2000)})})(this)" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-sm text-slate-500 dark:text-slate-400 hover:text-primary hover:border-primary/30 dark:hover:text-primary-400 dark:hover:border-primary-700/40">\
