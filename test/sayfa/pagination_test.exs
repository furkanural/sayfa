defmodule Sayfa.PaginationTest do
  ...
  describe "paginate/2" do
    ...
    test "returns single page when items fit" do
      pages = Pagination.paginate([1, 2, 3], page_size: 10, base_path: "/articles")
      ...
      assert page.url == "/articles/"
    end

    test "splits items across pages" do
      pages = Pagination.paginate(Enum.to_list(1..5), page_size: 2, base_path: "/articles")
    ...
    test "generates correct page numbers" do
      pages = Pagination.paginate(Enum.to_list(1..10), page_size: 3, base_path: "/articles")

      assert Enum.map(pages, & &1.page_number) == [1, 2, 3, 4]
    end

    test "sets total_items and total_pages" do
      pages = Pagination.paginate(Enum.to_list(1..7), page_size: 3, base_path: "/articles")
  ...
  describe "URL generation" do
    test "page 1 uses base path" do
      [page] = Pagination.paginate([1], page_size: 10, base_path: "/articles")
      assert page.url == "/articles/"
    end

    test "page 2+ uses /page/N/ suffix" do
      [_p1, p2, p3] =
        Pagination.paginate(Enum.to_list(1..6), page_size: 2, base_path: "/articles")

      assert p2.url == "/articles/page/2/"
      assert p3.url == "/articles/page/3/"
    end

    test "prev_url for page 1 is nil" do
      [p1 | _] = Pagination.paginate(Enum.to_list(1..5), page_size: 2, base_path: "/articles")
      assert p1.prev_url == nil
    end

    test "prev_url for page 2 points to base path" do
      [_p1, p2 | _] =
        Pagination.paginate(Enum.to_list(1..5), page_size: 2, base_path: "/articles")

      assert p2.prev_url == "/articles/"
    end

    test "prev_url for page 3+ uses /page/N/" do
      [_, _, p3] = Pagination.paginate(Enum.to_list(1..6), page_size: 2, base_path: "/articles")
      assert p3.prev_url == "/articles/page/2/"
    end

    test "next_url for last page is nil" do
      pages = Pagination.paginate(Enum.to_list(1..3), page_size: 2, base_path: "/articles")
    ...
    test "next_url for first page points to page 2" do
      [p1 | _] = Pagination.paginate(Enum.to_list(1..5), page_size: 2, base_path: "/articles")
      assert p1.next_url == "/articles/page/2/"
    end

    test "uses default page_size of 10" do
      pages = Pagination.paginate(Enum.to_list(1..25), base_path: "/articles")
      assert length(pages) == 3
